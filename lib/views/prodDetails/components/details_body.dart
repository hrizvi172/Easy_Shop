...
class _BodyState extends State<DetailsBody> {
  String size = 'M'; // Default to M for clothing, will be updated for shoes
  late User? user;
  ...
  void initState() {
    ...
    if (widget.product.isShoesProduct()) {
      size = '39';
    } else {
      size = 'M';
    }
                      ...
                      Flexible(
                        ...
                        child: TopRoundedContainer(
                          ...
                          child: Column(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              // Only show size selector for clothing and shoes
                              // if (widget.product.needsSizeSelector() ||
                              //     widget.product.isShoesProduct())
                              //   Flexible(
                              //     flex: 5,
                              //     child: widget.product.isShoesProduct()
                              //         ? Row(
                              //             crossAxisAlignment:
                              ...
                              //   )
                              // else
                              //   // Add spacing when no size selector
                              //   Flexible(
                              //     flex: 5,
                              //     child: SizedBox(
  ...
  void onPressedIconWithText(GlobalVars gv, UserInfoViewModel u) async {
    ...
    if (connection) {
      try {
        // For products without size selector, use a default size
        String selectedSize =
            (widget.product.needsSizeSelector() ||
                widget.product.isShoesProduct())
            ? size
            : 'One Size';

        String temp = widget.product.id + selectedSize;
        List<String> tempLsit = [];

        for (int i = 0; i < gv.userCart.length; i++) {
          tempLsit.add(gv.userCart[i].uid);
        }
        if (!tempLsit.contains(temp) || (user?.isAnonymous ?? false)) {
          gv.addToUserCart(widget.product, 1, selectedSize);
          u.addToCart(widget.product.id, selectedSize, 1);
          setState(() {
      ...
      child: Padding(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: PrimaryColor.withValues(alpha: size == s ? 1 : 0),
            ),
          ),
          child: Center(
            child: Text(
