...
class _UserInfoScreenState extends State<UserInfoScreen> {
  ...
  void initState() {
    ...
    _futureUserInfo = Provider.of<GlobalVars>(
      context,
      listen: false,
    ).getUserInfo(_u);
    super.initState();
  ...
  TextFormField buildAddressFormField(String LabelText) {
    return TextFormField(
      ...
      onSaved: (newValue) => _address = newValue ?? _address,
      decoration: InputDecoration(
        labelText: LabelText,
  ...
  TextFormField buildPhoneNumberFormField(String LabelText) {
    return TextFormField(
      ...
      onSaved: (newValue) => _phoneNumber = newValue ?? _phoneNumber,
      onChanged: (value) {
      ...
      validator: (value) {
        if (value?.isNotEmpty ??
            true && !phoneNumValidatorRegExp.hasMatch(value ?? '')) {
  ...
  TextFormField buildFullNameFormField(String LabelText) {
    return TextFormField(
      style: const TextStyle(fontWeight: FontWeight.w800),
      onSaved: (newValue) => _fullName = newValue ?? _fullName,
      onChanged: (value) {
      ...
      validator: (value) {
        if (value?.isNotEmpty ??
            true && !nameValidatorRegExp.hasMatch(value ?? '')) {
  ...
  List<DropdownMenuItem<String>> getDropdownItems() {
    ...
    for (int i = 0; i < governorates.length; i++) {
      ...
      var newItem = DropdownMenuItem<String>(child: Text(gov), value: gov);
